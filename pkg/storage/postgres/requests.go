...
func (pg *Postgres) Save(ctx context.Context, chatId int64, accessToken string, databaseURL string) error {
...
func (pg *Postgres) Get(chatId int64, databaseURL string) (accessToken string, err error) {
	db := getConn(databaseURL)
	defer db.Close()
